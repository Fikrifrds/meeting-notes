...
async fn transcribe_with_assemblyai(
    ...
    let upload_response = client
        ...
        .send()
        .await
        .map_err(|e| format!("Failed to upload audio file: {}", e))?;
    ...
    let upload_result: HashMap<String, serde_json::Value> = upload_response
        .json()
        .await
    ...
    let audio_url = upload_result
        .get("upload_url")
        .and_then(|v| v.as_str())
        .ok_or("No upload_url in response")?;
    
    // Step 2: Request transcription
    let speaker_labels_str = enable_speaker_diarization.to_string();
    let mut transcription_request = HashMap::new();
    transcription_request.insert("audio_url", audio_url);
    transcription_request.insert("speaker_labels", &speaker_labels_str);
    ...
    let transcription_response = client
        ...
        .send()
        .await
        .map_err(|e| format!("Failed to request transcription: {}", e))?;
    
    if !transcription_response.status().is_success() {
        let error_text = transcription_response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
        return Err(format!("Transcription request failed: {}", error_text));
    }
    
    let transcription_result: HashMap<String, serde_json::Value> = transcription_response
        .json()
        .await
        .map_err(|e| format!("Failed to parse transcription response: {}", e))?;
    
    let transcript_id = transcription_result
        .get("id")
        .and_then(|v| v.as_str())
    ...
    loop {
        ...
        let poll_response = client
            ...
            .send()
            .await
            .map_err(|e| format!("Failed to poll transcription status: {}", e))?;
        
        if !poll_response.status().is_success() {
            let error_text = poll_response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
            return Err(format!("Polling failed: {}", error_text));
        }
        
        let poll_result: HashMap<String, serde_json::Value> = poll_response
            .json()
            .await
        ...
        match status {
            ...
            "error" => {
                ...
                return Err(format!("AssemblyAI transcription error: {}", error_msg));
            }
            _ => {
